...
class WorldPoseFutureConfig:
    ...
    observed_pose_expert: bool = False
    observed_pose_rescue: bool = False
    observed_pose_rescue_alpha: float = 0.7
    observed_pose_rescue_min: float = 0.9
    observed_pose_rescue_world_min: float = 0.95
    ...
    # Attach a causal future-Pose warning head while preserving the existing
    # visual confirmation path. Unlike ``future_pose_led``, this never
    # replaces ``final_logits`` and is safe to add to a deployed checkpoint.
    future_pose_warning_head: bool = False
...
class ShortPoseMotionEncoder(nn.Module):
    ...
    def forward(
        ...
        if use_future_context:
            future = future_pose_tokens.reshape(
                future_pose_tokens.size(0), -1, future_pose_tokens.size(-1)
            )
            attended, _ = self.future_attention(summary, future, future, need_weights=False)
        else:
            attended = torch.zeros_like(summary)
...
class WorldPoseVisualFusion(nn.Module):
    ...
    def __init__(self, config: WorldPoseFutureConfig | None = None):
        ...
        self.future_pose_risk_head = (
            ...
            if (
                self.config.future_pose_led
                or self.config.future_pose_warning_head
                or self.config.dual_pose_heads
            )
            else None
        )
    ...
    def forward(
        ...
        if self.config.phase_motion_gate:
            short_pose_embedding, short_pose_event_logits, short_pose_phase_logits = (
                self.short_pose_motion_encoder(
                    ...
                    bbox_context,
                    predicted_pose,
                    use_future_context=not self.config.observed_pose_expert,
                )
            )
        target_pose = None
        ...
        if self.config.observed_pose_expert:
            # This expert is intentionally independent of JEPA future tokens.
            # It is the deployment-safe Pose counterpart to the World expert.
            short_pose_expert_logits = short_pose_event_logits
        elif self.short_pose_logit_delta is not None:
        ...
        legacy_pose_expert_gate = pose_expert_gate
        if self.config.phase_motion_gate:
        ...
        observed_pose_fusion_probability = torch.zeros_like(world_logodds)
        pose_world_agreement = torch.zeros_like(world_logodds, dtype=torch.bool)
        if self.config.dual_pose_heads:
            ...
            if self.config.observed_pose_rescue:
                legacy_logodds = (
                    (1.0 - legacy_pose_expert_gate) * world_logodds
                    + legacy_pose_expert_gate * pose_logodds
                )
                legacy_probability = torch.sigmoid(legacy_logodds)
                observed_probability = torch.sigmoid(final_logodds)
                observed_pose_fusion_probability = observed_probability
                world_probability = torch.sigmoid(world_logodds)
                pose_world_agreement = (
                    (observed_probability >= float(self.config.observed_pose_rescue_min))
                    & (world_probability >= float(self.config.observed_pose_rescue_world_min))
                )
                agreement = pose_world_agreement.to(legacy_probability.dtype)
                rescued_probability = legacy_probability + agreement * float(
                    self.config.observed_pose_rescue_alpha
                ) * torch.relu(observed_probability - legacy_probability)
                rescued_logodds = torch.logit(rescued_probability.clamp(1e-5, 1.0 - 1e-5))
                final_logits = torch.stack(
                    (torch.zeros_like(rescued_logodds), rescued_logodds), dim=1
                )
        ...
        return {
            ...
            "legacy_pose_expert_gate": legacy_pose_expert_gate,
            "observed_pose_fusion_probability": observed_pose_fusion_probability,
            "pose_world_agreement": pose_world_agreement,
